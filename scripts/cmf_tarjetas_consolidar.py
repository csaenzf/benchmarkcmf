...
INPUT_DIR  = BASE_DIR / "data" / "manual"
...
# ============================================================
# MAPEO DE FUSIONES / GRUPO COMERCIAL ACTUAL
# ============================================================
# Mapea Emisor_Comercial → el grupo vigente post-fusión.
# Permite agregar series históricas de entidades que ya no existen.
# Si un emisor NO aparece acá, Grupo_Comercial = Emisor_Comercial (sin cambio).
GRUPO_COMERCIAL = {
    # Corpbanca fue absorbido por Itaú (2016)
    "Corpbanca":        "Itaú",
    # BICE se fusionó con Security (2024)
    "BICE":             "Security",
    # Banco París fue absorbido por Scotiabank (legacy)
    "Banco París":      "Scotiabank",
    # BBVA fue absorbido por Scotiabank (2016)
    "BBVA":             "Scotiabank",
    # HSBC fue absorbido por Itaú (2014)
    "HSBC":             "Itaú",
    # Itaú legacy (*)
    "Itaú (legacy)":    "Itaú",
}
...
def read_cmf_sheet(filepath, sheet_name, kpi_name, producto, tipo_inst,
    ...
    melted["_Archivo_Fuente"] = Path(filepath).name
    melted["_Hoja_Fuente"] = sheet_name
    return melted
...
def read_agg_titadic(filepath, sheet_name, kpi_name, producto, tipo_inst):
    ...
    for col in data.columns[1:]:
        ...
        temp["Unidad"] = unidad
        temp["_Archivo_Fuente"] = Path(filepath).name
        temp["_Hoja_Fuente"] = sheet_name
        results.append(temp)
...
def post_process(df):
    ...
    # --- 9. Asignar Grupo Comercial (fusiones) ---
    df["Grupo_Comercial"] = df["Emisor_Comercial"].map(
        lambda x: GRUPO_COMERCIAL.get(x, x)
    )
...
# ============================================================
# CÓMPUTO Y VALIDACIÓN DE TOTALES
# ============================================================
# Claves de agrupación: todas las dimensiones EXCEPTO Titular_Adicional y Valor_KPI
TOTAL_GROUP_KEYS = [
    "Fecha", "Emisor_Comercial", "Emisor_Formal", "Marca", "Grupo_Comercial",
    "Tipo_Institucion", "Producto", "Apertura", "Nombre_KPI", "Unidad",
]

# KPIs donde NO tiene sentido sumar Titular + Adicional para obtener Total
# (porcentajes, ratios, índices)
KPI_SKIP_TOTAL = {
    ...
    "% Mora 30-89 dias",
    "% Mora 90-179 dias",
    "% Mora 180 dias a 1 año",
}


def compute_and_validate_totals(df):
    """
    Para cada combinación de dimensiones (Fecha × Emisor × KPI × ...):
      1. Si existen Titular y Adicional pero NO Total → crea Total = Tit + Adic
      2. Si existen los tres → valida Total == Tit + Adic y reporta discrepancias
      3. Ignora KPIs que son porcentajes/índices (no tiene sentido sumarlos)

    Returns:
        df_out: DataFrame con los Totales añadidos
        report: dict con estadísticas de lo que se hizo
    """
    report = {"created": 0, "validated": 0, "mismatches": [], "skipped_pct": 0}

    # Separar por Titular_Adicional
    df_tit = df[df["Titular_Adicional"] == "Titular"]
    df_adi = df[df["Titular_Adicional"] == "Adicional"]
    df_tot = df[df["Titular_Adicional"] == "Total"]

    if df_tit.empty or df_adi.empty:
        return df, report

    # Join Titular + Adicional por las claves de grupo
    merged = df_tit.set_index(TOTAL_GROUP_KEYS)[["Valor_KPI"]].rename(
        columns={"Valor_KPI": "Tit"}
    ).join(
        df_adi.set_index(TOTAL_GROUP_KEYS)[["Valor_KPI"]].rename(
            columns={"Valor_KPI": "Adi"}
        ),
        how="inner",
    )
    ...
    merged["Calc_Total"] = merged["Tit"] + merged["Adi"]

    # Verificar cuáles ya tienen Total en la base
    existing_totals = df_tot.set_index(TOTAL_GROUP_KEYS)[["Valor_KPI"]].rename(
        columns={"Valor_KPI": "Tot_Base"}
    )

    merged = merged.join(existing_totals, how="left")
    merged["Has_Total"] = merged["Tot_Base"].notna()

    # --- VALIDAR existentes ---
    with_total = merged[merged["Has_Total"]]
    if len(with_total) > 0:
        tolerance = 0.1
        diff = abs(with_total["Calc_Total"] - with_total["Tot_Base"])
        ok = (diff <= tolerance).sum()
        bad = diff[diff > tolerance]
        report["validated"] = int(ok)
        if len(bad) > 0:
            for idx in bad.index[:20]:  # max 20 para no saturar
                report["mismatches"].append({
                    "keys": dict(zip(TOTAL_GROUP_KEYS, idx if isinstance(idx, tuple) else (idx,))),
                    "calc": merged.loc[idx, "Calc_Total"],
                    "base": merged.loc[idx, "Tot_Base"],
                    "diff": bad.loc[idx],
                })

    # --- CREAR faltantes ---
    missing = merged[~merged["Has_Total"]]

    # Filtrar KPIs de porcentaje
    missing_reset = missing.reset_index()
    skip_mask = missing_reset["Nombre_KPI"].isin(KPI_SKIP_TOTAL)
    report["skipped_pct"] = int(skip_mask.sum())
    missing_to_create = missing_reset[~skip_mask]

    new_rows = []
    if len(missing_to_create) > 0:
        for _, row in missing_to_create.iterrows():
            new_row = {k: row[k] for k in TOTAL_GROUP_KEYS}
            new_row["Titular_Adicional"] = "Total"
            new_row["Valor_KPI"] = row["Calc_Total"]
            # Trazabilidad: marcar como calculado (nombres internos pre-rename)
            new_row["_Archivo_Fuente"] = "CALCULADO"
            new_row["_Hoja_Fuente"] = "Tit+Adic"
            new_row["Nombre_KPI_Original"] = f"Total calculado: {row['Nombre_KPI']} (Titular + Adicional)"
...
def main():
    ...
    # === VALIDAR Y COMPUTAR TOTALES ===
    print("📐 Validando y computando Totales (Titular + Adicional)...")
    consolidated, totals_report = compute_and_validate_totals(consolidated)

    # Rename traceability columns before selecting finals
    consolidated = consolidated.rename(columns={
        "_Archivo_Fuente": "Archivo_Fuente",
        "_Hoja_Fuente": "Hoja_Fuente",
        "Nombre_KPI_Original": "KPI_Original",
    })
    ...
    # Copia adicional para el sitio web
    WEBSITE_DIR = Path(r"C:\Dropbox\CMF_API\CMF_Reportes_Excel\CMF_Benchmark_Website")
    WEBSITE_DIR.mkdir(parents=True, exist_ok=True)
    shutil.copy2(output_path, WEBSITE_DIR / OUTPUT_FILE)

    elapsed = (datetime.now() - start).total_seconds()

    print(f"\n{'=' * 60}")
    print(f"✅ Consolidado v3.0 generado exitosamente")
    ...
    print(f"   🏢 Grupos:       {consolidated['Grupo_Comercial'].nunique()}")
    ...
    print(f"   📋 Copia web:    {WEBSITE_DIR / OUTPUT_FILE}")
    print(f"   ⏱  Tiempo:       {elapsed:.1f}s")
    print(f"{'=' * 60}")

    # Reporte de Totales
    print(f"\n📐 Reporte de Totales Titular + Adicional:")
    print(f"   ✅ Totales validados (coinciden con base):  {totals_report['validated']:,}")
    print(f"   ➕ Totales creados (no existían en base):   {totals_report['created']:,}")
    print(f"   ⏭️  Omitidos (% / índices, no se suman):    {totals_report['skipped_pct']:,}")
    if totals_report["mismatches"]:
        print(f"   ⚠ Discrepancias detectadas:               {len(totals_report['mismatches'])}")
        for m in totals_report["mismatches"][:5]:
            k = m["keys"]
            print(f"      {k.get('Fecha',''):%Y-%m} | {k.get('Emisor_Comercial','')} | {k.get('Nombre_KPI','')}")
            print(f"        Base: {m['base']:,.2f} vs Calculado: {m['calc']:,.2f} (diff: {m['diff']:,.2f})")
    else:
        print(f"   ✅ Sin discrepancias en totales existentes")

    # Reporte de Grupos Comerciales (fusiones)
    fusionados = {k: v for k, v in GRUPO_COMERCIAL.items()
                  if k in consolidated["Emisor_Comercial"].unique()}
    if fusionados:
        print(f"\n🔀 Fusiones mapeadas en Grupo_Comercial:")
        for orig, grupo in sorted(fusionados.items()):
            n = len(consolidated[consolidated["Emisor_Comercial"] == orig])
            print(f"   {orig:20s} → {grupo:15s} ({n:,} filas)")
